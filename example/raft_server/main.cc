...
static void on_message_from(Client *c) {
  ...
  if (c->msg.meaning == MEAN_SET) {
    ...
    debug("emit update: %s\n", buf);
    Update update = {static_cast<int>(strlen(buf)), buf, NULL};
    index =g_raft->emit(update); /* raft will copy the data */
    if (index < 0) {
...
static void drop_bads(void) {
  for (int i = 0; i < MAX_CLIENTS; i++) {
    Client *c = server.clients + i;
    if (c->state == CLIENT_DEAD) continue;
    if ((c->state == CLIENT_SICK) || !g_raft->isLeader()) {
      remove_client(c);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "hi:r:l:")) != -1) {
    ...
    case 'r':
      ...
      if (!raft.peerUp(id, host, port, id == server.id)) {
        usage(argv[0]);
        return EXIT_FAILURE;
      }
      if (id == server.id) {
  ...
  server.raftsock = raft.createUdpSocket();
